...
const cartoon: Filter = (pixels, amount = 125) => {
  ...
  for (let i = 0; i < size; i += 4) {
    outBuffer[i] = truncate(2 * (edgeBuffer[i] / 3 + rawBuffer[i] - amount) + amount);
    outBuffer[i + 1] = truncate(2 * (edgeBuffer[i + 1] / 3 + rawBuffer[i + 1] - amount) + amount);
    outBuffer[i + 2] = truncate(2 * (edgeBuffer[i + 2] / 3 + rawBuffer[i + 2] - amount) + amount);
